...
igniter, then venting the result into a CO2 storage tank. The work is split
...
| `CO2_OUTPUT_VENT` | `StructurePoweredVent` or `StructurePoweredVentLarge` | Removes chamber gas into the CO2 storage line. |
...
Install the CO2 output powered vent in the reactor chamber, with its pipe side
connected to the CO2 storage line. The exhaust IC sets the vent to inward mode
(`Mode 1`) and keeps it as the default emptying step before filling can begin.
...
the chamber into the storage line through `CO2_OUTPUT_VENT`. It then opens
...
to end, then writes exhaust mode to `CO2_REACTOR_PHASE`. The exhaust IC opens
the CO2 output vent until the chamber falls below `0.25 kPa`, then
writes ready mode so the next batch can start. If the tank sensor reads above
`9000 kPa`, the output vent stays off until tank pressure drops.
...
Keep the burn chamber small and isolated. The scripts control valve `On`, vent
`On`, vent `Mode`, and vent `PressureExternal`.
