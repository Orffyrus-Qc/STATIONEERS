...
et de l'O2, en envoyant une impulsion a un allumeur nomme, puis en evacuant le
...
| `CO2_OUTPUT_VENT` | `StructurePoweredVent` ou `StructurePoweredVentLarge` | Retire le gaz de la chambre vers la ligne de stockage de CO2. |
...
Installez le powered vent de sortie CO2 dans la chambre du reacteur, avec son
cote tuyau connecte a la ligne de stockage de CO2. L'IC d'evacuation met le vent
en mode inward (`Mode 1`) et l'utilise comme etape d'evacuation par defaut avant
de permettre le remplissage.
...
`CO2_OUTPUT_VENT`. Il ouvre ensuite `CH4_VALVE` et `O2_VALVE` seulement jusqu'a
...
`CO2_REACTOR_PHASE`. L'IC d'evacuation ouvre le vent de sortie CO2 jusqu'a ce
...
plus de `9000 kPa`, le vent de sortie reste eteint jusqu'a ce que la pression
du reservoir baisse.
...
des valves, ainsi que `On`, `Mode` et `PressureExternal` du vent. Si une tres
