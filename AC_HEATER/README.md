...
pipe heater enable signal with a `TemperatureOutput2` safety lockout.

## Purpose

`AC_HEATER.ic10` watches the `Mode` and `TemperatureOutput2` of the
`StructureAirConditioner` that holds the IC10 chip. It mirrors Active mode to
every named `StructurePipeHeater` on the data network, unless the output
temperature is too high.

When the host Air Conditioner is Active, all pipe heaters named `HEATER` turn
on. If `TemperatureOutput2` rises above `200`, the heaters are forced off and
stay off until `TemperatureOutput2` falls to `180` or lower. When the Air
...
```ic10
l acMode db Mode
l outputTemp db TemperatureOutput2
sbn HEATER_TYPE HEATER_NAME On heaterOn
```

## Mode Behavior

| Air Conditioner `Mode` | TemperatureOutput2 | Heater state |
| --- | --- | --- |
| `0` Idle | Any value | Off |
| `1` Active | `180` or lower after lockout | On |
| `1` Active | Above `200` | Off and locked out |
| `1` Active | Between `180` and `200` | Keeps previous lock state |
...
| --- | --- | --- |
| `TEMP_STOP` | `200` | Lock heaters off when `TemperatureOutput2` is above this. |
| `TEMP_RESUME` | `180` | Allow heaters again at or below this value. |
...
- The 200/180 limits use the units reported by `TemperatureOutput2`.
